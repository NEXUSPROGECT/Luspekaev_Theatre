...
async function fetchEventData(eventId) {
    ...
    try {
        ...
        event.times.forEach(time => {
            ...
            row.innerHTML = `
                   <td class="px-4 py-2">${day} ${month}</td>
                   <td class="px-4 py-2">${hours}:${minutes}</td>
                   <td class="px-4 py-2">${event.times[timesID].availableSeatsCount}</td>
                   <td class="px-4 py-2">
                       <a href="/ticket/${event.times[timesID].id}" class="bg-sky-500 text-white py-2 px-4 rounded shadow hover:bg-sky-700 transition rounded-full">Купить билет</a>
                   </td>
               `;
            timesID++;
        });
