  ...
  events.forEach(event => {
    ...
    item.innerHTML = `
      <div class="flex flex-col justify-center items-center text-center h-full bg-black bg-opacity-50 p-4 sm:p-16">
        <h2 class="text-white text-5xl font-bold mb-8 lg:w-1/3">${event.name}</h2>
        <h3 class="text-white text-xl sm:text-lg font-bold mb-8 max-w-[80%] sm:max-w-[60%] lg:w-1/3">${event.description}</h3>
        <a href="/event/${event.event.id}" class="px-6 py-2 bg-sky-500 font-bold text-white rounded-full hover:bg-sky-600">Подробнее</a>
      </div>
    `;
